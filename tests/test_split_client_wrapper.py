...
class TestSplitClientWrapper(unittest.TestCase):
    ...
    def test_using_internal_splitclient(self):
        wrapper = SplitClientWrapper({"ReadyBlockTime": 1, "ApiKey": "localhost", "ConfigOptions": {"splitFile": "split.yaml"}})
        assert wrapper.split_client != None
        assert wrapper.is_sdk_ready()
    ...
    def test_sdk_not_ready(self):
        wrapper = SplitClientWrapper({"ReadyBlockTime": 0.1, "ApiKey": "api", "ConfigOptions": {}})        
        assert not wrapper.is_sdk_ready()

    def test_invalid_apikey(self):
        with self.assertRaises(AttributeError) as context:
            wrapper = SplitClientWrapper({"ApiKey": 123})

    def test_invalid_config(self):
        with self.assertRaises(AttributeError) as context:
            wrapper = SplitClientWrapper({"ApiKey": "123", "ConfigOptions": "234"})
